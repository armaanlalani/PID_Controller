...
arg = robot.dir(2,i)/robot.dir(1,i);
robot.heading = atan(arg);
...
% Lines 32-42 is a filter system to help minimize the effects of noise
% generated. The encoder readings for each motor are passed through a
% moving-average filter as shown above. While utilizing a filter can help
% provide more accurate results, the time efficiency of the system is
% compromised. The use of a filter depends on the particular scenario, if
% responsiveness is preferred over accuracy, the use of a filter may not be
% desired and vice versa. The filter can be turned on by setting the value
% of f in main_script.m to 1 --> the simulation takes longer run when the
% filter is turned on.
